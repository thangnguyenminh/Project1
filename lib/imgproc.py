...
class ColorFilter(object):
    ...
    def exit(self):
        while(bool(self.tokens)):
            self.tokens.popitem();
        del self.tokens;
        del self.emptyRes;
        self.filter = None;
        del self.filter;
        del self.filterString;
...
class Image(object):
    ...
    def _getMergedContour(self):
        ...
        image = []; ori_img = []; stack = []; 
        del image; del ori_img; del stack;
    ...
    def extractMinEnclosingCircle(self):
        ...
        """
        circle = cv2.minEnclosingCircle(self._getMergedContour()[0])
    ...
    def extractMinEnclosingRectangle(self):
        """
        :return: The area of the minimum enclosing rectangle enclosing the object..
        :rtype: int
        ...
        x, y, width, height = cv2.boundingRect(self._getMergedContour()[0])

        if self.conn:
            self._addColumn("rectangle_centerx")
            self._addColumn("rectangle_centery")
            self._addColumn("rectangle_width")
            self._addColumn("rectangle_height")
            self.conn.execute("update images set rectangle_centerx=?, rectangle_centery=?, rectangle_width=?, rectangle_heigh=?, where pegasusid=?", (x, y, width, height, self.dbid))
            self.conn.commit()
        else:
            return (width*height)
    ...
    def extractConvexHull(self):
        """
        :return: The area and perimeter of the convex hull.
        :rtype: int

        Returns the area and perimeter of the convex hull around all non black pixels in the image.
        ...
        hull = cv2.contourArea(
                 cv2.approxPolyDP(
                    cv2.convexHull(
                        self._getMergedContour()[0]
                    ), 0.001, True
                ))
        
        # Perimeter
         
        perimeter = cv2.arcLength(self._getMergedContour()[0], closed=True)            
        #perimeter = 0
        if self.conn:
            self._addColumn("convex_hull_area")
            self._addColumn("convex_hull_perimeter")
            self.conn.execute("update images set convex_hull_area=?, convex_hull_perimeter=? where pegasusid=?", (hull, perimeter, self.dbid))
            self.conn.commit()
        else:
            return hull, perimeter
    def extractLongestAxis(self):
        """
        :return: Longest Axis: line through center of mass and point on the convex hull that is furthest away.
        :rtype: int

        """
        # Moments
        #  m = cv2.moments(obj)
        m = self.extractMoments()
        # Centroid (center of mass x, center of mass y)
        cmx, cmy = (m['m10'] / m['m00'], m['m01'] / m['m00'])
        
        # Convex Hull
        
        hull = cv2.convexHull(self._getMergedContour()[0])
        
        ix, iy, iz = np.shape(self.image)
        ...
        if self.conn:
            self._addColumn("Longest Axis")
            self.conn.execute("update images set Longest Axis=? where pegasusid=?", (caliper_length, self.dbid))
            self.conn.commit()
            return
        else:
            return caliper_length
